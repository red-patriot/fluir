...
const mockOptions: Completion[] = [
  {
    short_name: "+ binary",
    description: "Add",
    data: { kind: "operator", arity: 2 },
  },
  {
    short_name: "- unary",
    description: "Negate",
    data: { kind: "operator", arity: 1 },
  },
  {
    short_name: "int",
    description: "Integer",
    data: { kind: "constant", flType: "I32" },
  },
];
...
describe("CreateNodeDialog", () => {
  ...
  it("resets selection index when search text changes", async () => {
    ...
    expect(
      firstOption.parentElement!.closest("[data-accent-color]"),
    ).toHaveAttribute("data-accent-color", "blue");

    // Type to change search — selection should reset
    await userEvent.type(input, "i");
    const intOption = screen.getByText("int");
    expect(
      intOption.parentElement!.closest("[data-accent-color]"),
    ).toHaveAttribute("data-accent-color", "gray");
  });
  ...
  it("ArrowDown moves selection down through filtered options", async () => {
    ...
    expect(
      screen
        .getByText("+ binary")
        .parentElement!.closest("[data-accent-color]"),
    ).toHaveAttribute("data-accent-color", "blue");

    await userEvent.type(input, "{ArrowDown}");
    expect(
      screen.getByText("- unary").parentElement!.closest("[data-accent-color]"),
    ).toHaveAttribute("data-accent-color", "blue");
    expect(
      screen
        .getByText("+ binary")
  ...
  it("ArrowDown stops at last option", async () => {
    ...
    expect(
      screen.getByText("int").parentElement!.closest("[data-accent-color]"),
    ).toHaveAttribute("data-accent-color", "blue");
    expect(
      screen
        .getByText("+ binary")
  ...
  it("builds correct request for a constant completion", async () => {
    ...
    expect(mockEditProgram).toHaveBeenCalledWith({
      ...
      params: { discriminator: "constant", type: "I32" },
    });
  });

  it("builds correct request for a call completion", async () => {
    const callOptions: Completion[] = [
      { short_name: "myFunc", description: "", data: { kind: "call" } },
    ];
  ...
  it("renders options inside a scroll area", () => {
    const manyOptions: Completion[] = [
      {
        short_name: "op1 binary",
        description: "Op1",
        data: { kind: "operator", arity: 2 },
      },
      {
        short_name: "op2 binary",
        description: "Op2",
        data: { kind: "operator", arity: 2 },
      },
      {
        short_name: "op3 binary",
        description: "Op3",
        data: { kind: "operator", arity: 2 },
      },
      {
        short_name: "op4 binary",
        description: "Op4",
        data: { kind: "operator", arity: 2 },
      },
      {
        short_name: "op5 binary",
        description: "Op5",
        data: { kind: "operator", arity: 2 },
      },
      {
        short_name: "op6 binary",
        description: "Op6",
        data: { kind: "operator", arity: 2 },
      },
      {
        short_name: "op7 binary",
        description: "Op7",
        data: { kind: "operator", arity: 2 },
      },
      {
        short_name: "op8 binary",
        description: "Op8",
        data: { kind: "operator", arity: 2 },
      },
    ];
