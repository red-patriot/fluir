#ifndef FLUIR_COMPILER_TEST_BYTECODE_ASSERTIONS_HPP
#define FLUIR_COMPILER_TEST_BYTECODE_ASSERTIONS_HPP
...
#define EXPECT_BC_VALUES_EQ(expectedValues, actualValues)                                         \
  ...
  for (size_t bc_values_eq_i = 0; bc_values_eq_i != expectedValues.size(); ++bc_values_eq_i) {    \
    EXPECT_EQ(expectedValues[bc_values_eq_i], actualValues.at(bc_values_eq_i)) << bc_values_eq_i; \
  }
