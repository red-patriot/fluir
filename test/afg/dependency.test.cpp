...
TEST(TestDependency, DereferenceToGetSourceNode) {
  ...
  auto actual = fafg::makeDependency<fafg::DoubleConstant>(4.0);

  EXPECT_TRUE(expected.equals(*actual));
}

TEST(TestDependency, DereferenceToGetSourceNodeConst) {
  auto expected = fafg::DoubleConstant(4.0);

  const auto actual = fafg::makeDependency<fafg::DoubleConstant>(4.0);
...
TEST(TestDependency, ArrowOperatorWorksAsExpected) {
  ...
  auto actual = fafg::makeDependency<fafg::DoubleConstant>(3.5);
...
TEST(TestDependency, ArrowOperatorWorksAsExpectedWhenConst) {
  ...
  const auto actual = fafg::makeDependency<fafg::DoubleConstant>(3.5);
...
TEST(TestDependency, CanGetUnderlyingPointer) {
  ...
  auto actual = fafg::makeDependency<fafg::DoubleConstant>(0.5);
...
TEST(TestDependency, CanGetUnderlyingConstPointer) {
  ...
  const auto actual = fafg::makeDependency<fafg::DoubleConstant>(0.5);

  EXPECT_TRUE(actual.get()->equals(expected));
}

TEST(TestDependency, CopyConstructingSharesDependency) {
  auto first = fafg::makeDependency<fafg::DoubleConstant>(-12.432);
  auto second = first;
...
TEST(TestDependency, CopyAssigningSharesDependency) {
  auto first = fafg::makeDependency<fafg::UnaryOp>(
      fluir::Operator::PLUS,
      fafg::makeDependency<fafg::DoubleConstant>(10.6));
  auto second = fafg::makeDependency<fafg::DoubleConstant>(12345.0);
...
TEST(TestDependency, MoveAssigningMakesACopy) {
  auto first = fafg::makeDependency<fafg::DoubleConstant>(6.5467);
  auto second = fafg::makeDependency<fafg::DoubleConstant>(12345.0);
