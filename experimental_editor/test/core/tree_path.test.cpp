...
namespace {
  ...
  fluir::editor::et::ParseTree makeTree() {
    ...
    fn.body.nodes.emplace(
      10, fluir::editor::et::Constant{.id = 10, .location = kNodeLoc, .value = fluir::literals_types::I32{0}});
    fn.body.conduits.emplace(40, fluir::editor::et::Conduit{.id = 40, .input = 10});

    fluir::editor::et::FunctionDecl empty;
    empty.id = 2;

    fluir::editor::et::ParseTree tree;
    tree.declarations.emplace(1, fluir::editor::et::Declaration{fn});
    tree.declarations.emplace(2, fluir::editor::et::Declaration{empty});
    tree.declarations.emplace(
      3, fluir::editor::et::Declaration{fluir::editor::et::Comment{.id = 3, .location = kCommentLoc}});
    return tree;
  }

  // Param 2 (I32) and return 4 (F64).
  fluir::editor::et::FunctionDecl makeRailedFunction() {
    fluir::editor::et::FunctionDecl fn;
    fn.id = 1;
    fn.input = fluir::editor::et::FunctionDecl::InputBlock{
      .parameters = {{.id = 2, .index = 0, .name = "a", .typeName = "I32"}}};
...
TEST(TreePath, FunctionAtResolvesAOneSegmentPath) {
  ...
  const fluir::editor::et::FunctionDecl* fn = functionAt(tree, FullID{1});
...
TEST(TreePath, DeclarationAtResolvesFunctionsAndComments) {
  ...
  const fluir::editor::et::Declaration* fn = declarationAt(tree, FullID{1});
  const fluir::editor::et::Declaration* comment = declarationAt(std::as_const(tree), FullID{3});

  ASSERT_NE(fn, nullptr);
  ASSERT_NE(comment, nullptr);
  EXPECT_TRUE(std::holds_alternative<fluir::editor::et::FunctionDecl>(*fn));
  EXPECT_TRUE(std::holds_alternative<fluir::editor::et::Comment>(*comment));
}
...
TEST(TreePath, BlockOfAFunctionIsItsBody) {
  ...
  const fluir::editor::et::Block* block = blockOf(tree, FullID{1});
...
TEST(TreePath, NodeAtResolvesANodeInsideItsParentsBlock) {
  ...
  const fluir::editor::et::Node* node = nodeAt(tree, FullID{1, 10});

  ASSERT_NE(node, nullptr);
  EXPECT_TRUE(std::holds_alternative<fluir::editor::et::Constant>(*node));
}
...
TEST(TreePath, RailTypeAtFindsAParamType) {
  fluir::editor::et::FunctionDecl fn = makeRailedFunction();
...
TEST(TreePath, RailTypeAtMissesUnknownIds) {
  ...
  EXPECT_EQ(railTypeAt(fluir::editor::et::FunctionDecl{}, 2), nullptr);
}
...
namespace {
  ...
  fluir::editor::et::Constant makeConstant(fluir::ID id, FlowGraphLocation location) {
    return fluir::editor::et::Constant{.id = id, .location = location, .value = fluir::literals_types::I32{0}};
  }
  ...
  fluir::editor::et::ParseTree makeNestedTree() {
    fluir::editor::et::Block thenBranch;
    thenBranch.nodes.emplace(1, makeConstant(1, kThenNodeLoc));
    thenBranch.conduits.emplace(50, fluir::editor::et::Conduit{.id = 50, .input = 1});

    fluir::editor::et::Block elseBranch;
    elseBranch.nodes.emplace(1, makeConstant(1, kElseNodeLoc));

    fluir::editor::et::Conditional conditional{.id = 20,
                                               .location = kCondLoc,
                                               .condition = {},
                                               .inputs = {},
                                               .outputs = {},
    ...
    fluir::editor::et::ParseTree tree = makeTree();
    std::get<fluir::editor::et::FunctionDecl>(tree.declarations.at(1)).body.nodes.emplace(20, conditional);
    return tree;
  }
...
TEST(TreePath, BlockOfABranchPathIsThatBranch) {
  fluir::editor::et::ParseTree tree = makeNestedTree();

  const fluir::editor::et::Block* thenBlock = blockOf(tree, FullID{1, 20, THEN_BRANCH_ID});
  const fluir::editor::et::Block* elseBlock = blockOf(tree, FullID{1, 20, ELSE_BRANCH_ID});
...
TEST(TreePath, NodeAtResolvesANestedNode) {
  ...
  const fluir::editor::et::Node* node = nodeAt(tree, FullID{1, 20, THEN_BRANCH_ID, 1});

  ASSERT_NE(node, nullptr);
  EXPECT_EQ(std::get<fluir::editor::et::Constant>(*node).location, kThenNodeLoc);
}
...
TEST(TreePath, BranchAtResolvesEitherBranch) {
  ...
  const fluir::editor::et::Block* thenBranch = branchAt(tree, FullID{1, 20, THEN_BRANCH_ID});
  const fluir::editor::et::Block* elseBranch = branchAt(tree, FullID{1, 20, ELSE_BRANCH_ID});

  ASSERT_NE(thenBranch, nullptr);
  ASSERT_NE(elseBranch, nullptr);
  EXPECT_TRUE(thenBranch->nodes.at(1) == fluir::editor::et::Node{makeConstant(1, kThenNodeLoc)});
  EXPECT_TRUE(elseBranch->nodes.at(1) == fluir::editor::et::Node{makeConstant(1, kElseNodeLoc)});
}
